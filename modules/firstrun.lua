pfUI:RegisterModule("firstrun", "vanilla:tbc", function ()
  ...
  local function CreateFirstRunPage()
    ...
    f.progress:SetStatusBarColor(.7,.2,1,1)
  ...
  pfUI.firstrun:AddStep("profile", function()
    local f = CreateFirstRunPage()
    f.text:SetText(T["A new installation of |cffbb33ffpf|rUI ships with 4 prebuilt design profiles. Click below if you wish to load one of these profiles."])
  ...
  pfUI.firstrun:AddStep("finalize", function()
    local f = CreateFirstRunPage()
    f.text:SetText(T["Your interface is now set up.\n\nFor advanced configuration, just open the |cffbb33ffpf|rUI settings via the escape menu or type \"|cffffffaa/pfui|r\" into the chat.\n\n Have a nice trip!\n\n|cffaaaaaa- Shagu"])
    return f
  end)
end)
